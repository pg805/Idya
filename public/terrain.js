...
(function() {
  ...
  function blitNamed(ctx, name, dx, dy, S) {
    const a = ATLAS[name];
    if (!a) return;
    blit(ctx, a[0], a[1], a[2], dx, dy, S);
  }
  ...
  // Draw `board` (a serialized CombatSession board, including its `terrain`)
  // onto `canvas`, sized so one board square is `cellPx` CSS pixels. Returns
  // false and leaves the canvas alone if there's nothing to draw yet.
  function paintTerrain(canvas, board, cellPx, onReady) {
    const terrain = board && board.terrain;
    if (!terrain) return false;
    if (!ready) { waiting.push(onReady); return false; }

    const w = terrain.width, h = terrain.height;

    // Backing store is an INTEGER multiple of the 32px art, so the art is never
    // resampled at a fractional scale. On a 2x display with 48px squares that
    // works out to an exact 3x; elsewhere the browser does a final nearest
    // rescale to fit, which pixel art survives far better than a blur.
    ...
    // 2. grass — autotiled on top, so dirt shows through as clearings and paths.
    paintMaterial(ctx, 'grass', w, h, (x, y) => terrain.ground[y][x] === 'g', S);

    // 3. grass overlay — tufts that break up the flat fill.
    for (const o of terrain.overlay) blitNamed(ctx, o.s, px(o.x), px(o.y), S);
    ...
    const props = terrain.obstacles.map((p) => {
      ...
      return { x: p.x, y: p.y, stack: dead ? [p.rubble] : p.stack, shadow: dead ? null : p.shadow };
    });

    // 4. shadows — under the props, over the ground.
    for (const p of props) if (p.shadow) blitNamed(ctx, p.shadow, px(p.x), px(p.y), S);

    // 5. decor — scatter, then the part of each prop that stands on its square.
    for (const s of terrain.scatter) blitNamed(ctx, s.s, px(s.x), px(s.y), S);
    for (const p of props) blitNamed(ctx, p.stack[0], px(p.x), px(p.y), S);
    ...
    for (const p of [...props].sort((a, b) => a.y - b.y))
      for (let i = 1; i < p.stack.length; i++)
        blitNamed(ctx, p.stack[i], px(p.x), px(p.y - i), S);

    // Square lines last, over the lot. This is a tactical grid before it's a
    // landscape — you have to be able to count squares at a glance — but it's
    // kept faint enough to read as ground markings rather than UI chrome.
