// View: Crafting Bench — Craft, Upgrade, Enchant tabs.
(function() {
  let data         = null;
  let activeProfs  = null;
  let openRecipe   = null;
  let activeInnerTab = 'craft';
  ...
  async function mount(root) {
    root.innerHTML = `
      ...
      <div id="page-tabs">
        ...
        <button class="page-tab" data-tab="enchant" onclick="Views.craft.showTab('enchant')">Enchant</button>
      </div>
      <section id="craft-tab">
        <div id="recipe-filter"></div>
        <div id="recipe-list"></div>
      </section>
      <section id="upgrade-tab" style="display:none">
        <div id="upgrade-header">
          <select id="weapon-picker" onchange="Views.craft.pickWeapon(this.value)"></select>
        </div>
        <div id="upgrade-panel"></div>
      </section>
      <section id="enchant-tab" style="display:none">
        <div id="enchant-header">
          <select id="enchant-weapon-picker" onchange="Views.craft.pickEnchantWeapon(this.value)"></select>
  ...
  function renderRecipeFilter() {
    ...
    el.innerHTML = Object.keys(PROF_LABEL).map(key => `
      <label class="filter-check">
        <input type="checkbox" ${activeProfs.has(key) ? 'checked' : ''} onchange="Views.craft.toggleProf('${key}')">
        ${PROF_LABEL[key]}
      </label>
    `).join('');
  }
  ...
  function renderRecipes() {
    ...
    for (const r of visible) {
      ...
      card.innerHTML = `
        <div class="recipe-row${isOpen ? ' open' : ''}" onclick="Views.craft.toggleRecipe('${r.id}')">
        ...
        ${isOpen ? `
          <div class="recipe-detail">
            ...
            <div class="craft-row">
              ...
              <button class="craft-btn" onclick="Views.craft.doCraft('${r.id}')" ${r.available ? '' : 'disabled'}>Craft</button>
            </div>
          </div>` : ''}
      `;
  ...
  function renderActionRow(a, atCap) {
    ...
    if (a.type === 'value') {
      ...
      const btn = (!atCap && !pendingDelta)
        ? `<button class="upg-btn" onclick="Views.craft.upgradeValue('${esc(a.name)}')">+1</button>` : '';
    ...
    const btn = (!atCap && !pendingDelta && !editingThis)
      ? `<button class="upg-btn" onclick="Views.craft.startFieldEdit('${esc(a.name)}', ${a.field_len})">Upgrade</button>` : '';
  ...
  function renderFieldEditor() {
    ...
    for (let i = 0; i < pendingDelta.delta.length; i++) {
      ...
      entriesHtml += `<div class="fe-entry">
        ...
        <div class="fe-btns">
          <button onclick="Views.craft.adjustDelta(${i}, -1)" ${canMinus ? '' : 'disabled'}>−</button>
          <button onclick="Views.craft.adjustDelta(${i},  1)" ${canPlus  ? '' : 'disabled'}>+</button>
        </div>
      </div>`;
    }
    ...
    el.innerHTML = `<div class="field-editor">
      ...
      <div class="fe-controls">
        <button onclick="Views.craft.cancelFieldEdit()">Cancel</button>
        <button class="upg-btn" onclick="Views.craft.confirmFieldEdit()" ${rem !== 0 ? 'disabled' : ''}>Confirm</button>
      </div>
    </div>`;
  }
  ...
  function renderEnchantRow(a, slotsFull) {
    ...
    if (enchanted) {
      ...
      extraHtml = `<button class="upg-btn" onclick="Views.craft.startEnchant('${esc(a.name)}')">Enchant</button>`;
    }
  ...
  function renderEnchantEditor(a) {
    ...
    if (a.type === 'field') {
      ...
      for (let i = 0; i < p.delta.length; i++) {
        ...
        entriesHtml += `<div class="fe-entry">
          <span class="fe-val">${effective}</span>
          <div class="fe-btns">
            <button onclick="Views.craft.adjEnchantDelta(${i}, -1)" ${canMinus ? '' : 'disabled'}>−</button>
            <button onclick="Views.craft.adjEnchantDelta(${i},  1)" ${canPlus  ? '' : 'disabled'}>+</button>
          </div>
        </div>`;
      }
    ...
    return `<div class="field-editor">
      <div class="enchant-dropdowns">
        <label>Kind:</label>
        <select onchange="Views.craft.setEnchantKind(this.value)">
          ${kinds.map(k => `<option value="${k}" ${k === p.kind ? 'selected' : ''}>${k}</option>`).join('')}
        </select>
        <label>Category:</label>
        <select onchange="Views.craft.setEnchantCategory(this.value)">
          ${cats.map(c => `<option value="${c}" ${c === p.category ? 'selected' : ''}>${c}</option>`).join('')}
        </select>
        <label>Subtype:</label>
        <select onchange="Views.craft.setEnchantSubtype(this.value)">
      ...
      <div class="fe-controls">
        <button onclick="Views.craft.cancelEnchant()">Cancel</button>
        <button class="upg-btn" onclick="Views.craft.confirmEnchant()" ${(!canAfford || !sumOk) ? 'disabled' : ''}>Confirm</button>
      </div>
    </div>`;
  }
  ...
  function unmount() {
    ...
    activeInnerTab = 'craft';
  }

  window.Views = window.Views ?? {};
  window.Views.craft = {
    mount, unmount,
    showTab, toggleProf, toggleRecipe, doCraft,
    pickWeapon, upgradeValue, startFieldEdit, adjustDelta, cancelFieldEdit, confirmFieldEdit,
    pickEnchantWeapon, startEnchant, cancelEnchant, confirmEnchant,
