...
- Action type IDs: 1=Strike, 2=Block, 3=Buff, 4=DOT, 5=Debuff, 6=Heal, 7=Reflect, 8=Shield, 9=Block Tile, 10=Buff Tile, 11=Hazard Tile, 12=Destroy Obstacle, 13=Slow Tile (leaving it costs +1 movement)
- **`Area: N`** is a general field on any action: tile actions (9/10/11/13) drop an N×N block; attacks/DOTs (1/4) become an N×N AOE. Geometry (`areaBlock` in `resolution.ts`): odd N centers on the target, even N uses the target as the top-left corner. Aimed-AOE hits all enemies in the block (cost paid once, no per-target crit); slow tiles add +1 leave-cost in `movement.ts`.
