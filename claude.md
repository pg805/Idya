...
### Painted terrain
The combat board is drawn with the 32px pixel-art tileset in six layers — dirt → grass → grass overlay → shadows → decor → above decor — across **two canvases that sandwich the DOM grid**: layers 1–5 on `#board-terrain` below the cells, layer 6 on `#board-canopy` above the tokens. The `.cell` divs are unchanged and just become transparent windows onto the ground, so highlight/token/targeting code never has to know terrain exists.

The canopy is above the tokens because a unit under a tree is genuinely behind the leaves. Two rules keep that readable, both in `paintTerrain`: the unit's **ring is redrawn on top** of the canopy, and **leaves lose to the UI** — squares the player can act on get the canopy thinned over them so a highlight is never buried.

Ground uses **dual-grid autotiling**: the 6 shapes per material (full/empty/edge/outer/inner/diagonal) × 4 rotations cover all 16 corner masks, with the drawing grid offset half a tile from the board grid. These are **forests** — the board is grass and dirt is a rare small scuff (~8%), not clearings; obstacles are ~80% trees. Obstacles are trees whose **trunk base sits on the blocked square**, stacking upward into the (still walkable) squares above; one too near the top edge falls back to a one-square prop. Bushes are obstacles and rare, never scatter — a scattered bush is indistinguishable from a canopy, and the rule has to stay *any big leafy mass is blocked*.
