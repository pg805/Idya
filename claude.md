...
- `src/tools/spatial_sim.ts` — Headless spatial sim: real engine + `choosePlan` driving **both** sides over real boards. `node lib/tools/spatial_sim.js [N] [enemy]` prints a win%/rounds/HP/timeout table (grades positional kits the non-spatial `simulate.ts` can't). `... debug <enemy> <weapon>` traces one battle; `... replay <enemy> <weapon>` writes `public/replay.json` for the dev replay viewer.
- **Dev AI replay** (`public/replay.html` + `replay.js`, reuses `game.css`) — step through a recorded sim battle turn-by-turn: the board like a real fight, plus the predicted-movement heatmap and every scored candidate plan (chosen highlighted). Open `/replay.html` after a `spatial_sim replay` run. (`public/replay.json` is gitignored — regenerate per run.)
